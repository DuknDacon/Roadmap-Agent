...
# 금액·기간 관련 용어(매달/월 저축액/목표 금액)는 값이 없어도 조건 변경으로
# 두는 편이 낫다 — "매달 얼마씩 저축해야 해?"는 결국 로드맵을 다시 보여주면
# 답이 되고, 값이 없을 때는 execute_conversation이 현재 조건으로 계산해
# 응답한다. 반면 "비상자금"은 값이 아니라 개념을 묻는 질문("비상자금은 왜
# 필요해?")이 흔한데, 그때 로드맵 재계산 결과만 나가 질문에 답하지 못했다.
# 그래서 비상자금만 "없다/있다" 같은 실제 답이 붙은 경우로 제한한다.
_CHANGE_TERMS = re.compile(
    r"(바꿔|변경|다시\s*계산|늘려|줄여|낮춰|높여|월\s*(?:저축액|투입액|예산)|"
    r"목표\s*(?:금액|액|시점)|비상\s*(?:금|자금).{0,8}(?:없|미보유|있|보유)|매달)"
)
...
_FINANCE_TERMS = re.compile(
    # 비상금/비상자금은 _CHANGE_TERMS에서 값 없이 언급만 된 경우 조건 변경이
    # 아니게 됐다. "비상자금은 왜 필요해?" 같은 개념 질문이 RAG로 가도록 여기
    # 포함한다.
    r"(ISA|연금|세액공제|비과세|적금|투자|채권|주식|금리|세금|비상\s*(?:금|자금))",
    re.I,
)
...
def execute_conversation(
    ...
    *,
    ...
    if plan.intent == ConversationIntent.FINANCIAL_QA:
        ...
        # 일반 금융 RAG 코퍼스가 애초에 커버하지 않는 질문이면 RAG가 실패했다는
        # 문구를 반환한다 — 이때 두 가지를 순서대로 시도한다.
        if any(marker in reply for marker in _FINANCIAL_QA_FAILURE_MARKERS):
            # (1) 개별 정책 고유 용어(예: "참여기업")는 이번 대화의 게이트
            # 문구에서 관련 조건 설명을 찾아본다.
            gate_matches = gate_registry.find_gates_mentioning(message) if gate_registry else []
            if gate_matches:
                reply = gate_hint_reply(gate_matches)
            elif explainer is not None and hasattr(explainer, "answer_general_definition"):
                # (2) "직계존비속이 뭐야?" 처럼 금융도 정책 고유 용어도 아닌
                # 일반 상식·법률·행정 용어 정의 질문은, 재무 자문이 아니라
                # 사전적 정의라 신뢰도 리스크가 낮으므로 공식 도메인 인용
                # 제한 없이 웹검색으로 답한다 — 그 제한 때문에 이런 질문에
                # 늘 "확정할 수 없다"고만 답하던 문제(실사용자 피드백 + 서버
                # 로그로 확인)를 여기서 해결한다.
                try:
                    general_answer = explainer.answer_general_definition(message)
                except Exception:
                    general_answer = None
                if general_answer:
